...
def getHomepageWithSearchpageAndHpidx(uid, searchpage, hpidx):
    ...
    soup = BeautifulSoup(html, "html5lib")
    try:
        srg = soup.find(attrs={'class', 'srg'})[hpidx]
...
def getPicFromHtml(html, name):
    ...
    for x in xrange(count - 1, -1, -1):
        ...
        try:
            ...
            alt = ''
        if 'logo' in src or 'logo' in alt or src[-3:] == 'svg':
            del imgs[x]
    try:
        return imgs[0]
    except:
...
def getEmailWithUser(uid, homepage, hpidx, html):
    ...
    for line in html.split('\n'):
        line = line.strip()
        if ('at' in line or 'AT' in line) and ('dot' in line or 'DOT' in line):
            return line
    for line in html.split('\n'):
        line = line.strip()
...
def getTrainFeas():
    print('getTrainFeas()')
    userlist = getUserlist(traindata=True)
    f_fea = open(utils.getPathFeaTraining(1), 'w')
...
def train():
    ...
    for i in xrange(len(X_vali)):
        for j in xrange(len(X_vali[i])):
    ...
    print(type(Pred_vali))
    print(Pred_vali.shape)
    ...
    for i, line in enumerate(uid_hpidx_lines):
        ...
        user_dic['email'] = getEmailWithUser(user_dic['id'], user_dic['homepage'], user_dic['hpidx'], html).encode('utf8')
        user_dic['gender'] = get_Gender(user_dic['name']).encode('utf8')
        user_dic['position'] = (';'.join(get_title(html))).encode('utf8')
        user_dic['location'] = get_location(html, user_dic['org']).encode('utf8')
        s = '%s\t%s\t%s\t%s\t%s\t%s\t%s\n' % (user_dic['id'], user_dic['homepage'], user_dic['gender'], user_dic['position'], user_dic['pic'], user_dic['email'], user_dic['location'])
        f_res_task1.write(s)
